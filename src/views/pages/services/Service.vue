...
const loadServices = async () => {
    ...
    servicesData.value = services.value.data;
};
...
const actions: Action[] = [
    {
        ...
        action: (item) => {
            ...
            form.value.process_fields = item.process_fields.map(field => field.id);
...
<template>
    ...
    <ModalComponent sizeClass="modal-service" :show="showModal" @closeModal="handleCloseModal" :titleHeader="handleModalTitle()" @submit="handleSubmit">
        ...
        <FormGroupComponent>
            <MultipleSelectComponent v-model="form.process_fields" :options="processSelectData" />
        </FormGroupComponent>
    </ModalComponent>
